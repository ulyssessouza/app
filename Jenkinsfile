...
pipeline {
    ...
    stages {
        ...
        stage('Publish image to DockerHub') {
            ...
            environment {
                DOCKERHUB_CREDS=credentials('jenkins-docker-hub-credentials')
            }
            steps {
                dir('src/github.com/docker/app') {
                    checkout scm
                    ...
                    script {
                        docker.withRegistry('https://registry.hub.docker.com', 'jenkins-docker-hub-credentials') {
                            sh 'make TAGNAME=${TAGNAME} -f docker.Makefile dockerhub-publish'
                            sh './update-dockerhub-repo.sh DockerHubRepo.json'
                        }
                    }
                }
            }
